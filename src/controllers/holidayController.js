...
// ✅ Create a holiday for a specific class
export const createClassSpecificHoliday = async (req, res) => {
  try {
    ...
    if (existing) {
      return res.status(400).json({
        message: `Holiday already exists for this class on this date: ${existing.title}`,
        existingHoliday: existing,
      });
    }
    ...
    res.status(201).json({ message: "Class-specific holiday created successfully", holiday });
  } catch (err) {
    res.status(500).json({ message: "Error creating class-specific holiday", error: err.message });
  }
};
