...
// Get a student's absent-day summary for the current session
export const getStudentAbsenceSummary = async (req, res) => {
  try {
    const { studentId } = req.params;
    ...
    const { startDate } = await getSessionDates();
    const today = new Date();
    ...
    const query = {
      studentId,
      status: "absent",
      date: { $gte: sessionStart, $lte: todayEnd }
    };

    if (classId) {
      if (!mongoose.Types.ObjectId.isValid(classId)) {
        return res.status(400).json({ message: "classId must be a valid ID" });
      }
      query.classId = classId;
    }

    const absentRecords = await Attendance.find(query)
      .select("date classId remarks")
    ...
    res.json({
      ...
      absentDates,
      absences: absentRecords.map(record => ({
    ...
    console.error("Error fetching student absence summary:", err);
    res.status(500).json({
      message: "Error fetching student absence summary",
      error: err.message
    });
  }
};
